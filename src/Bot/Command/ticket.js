...
module.exports = class extends Command {
	...
	run() {
		...
		this.Client.Client.guilds.cache.first().channels.create("Ticket #" + this.ticket.id, {topic: `Ticket #${this.ticket.id} opened by ${this.ticket.creatorTag}`, parent: "831574173464657931", permissionOverwrites: [{id:this.Client.Client.guilds.cache.first().roles.everyone, deny: ['VIEW_CHANNEL', 'SEND_MESSAGES']},{id:this.ticket.creator.id, allow: ['VIEW_CHANNEL', 'SEND_MESSAGES']}]})
			.then(channel => {
				...
				.then(w => {
					w.send({
						...
						commandMessage.reactionCollector = commandMessage.createReactionCollector(commandMessage.reactionFilter, { time: 900000 });
